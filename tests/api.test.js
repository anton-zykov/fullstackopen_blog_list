...
describe('blogs testing', () => {
  beforeEach(async () => {
    ...
    const savedUser = await api.post('/api/users').send(helper.initialUser)
    await Blog.deleteMany({})
    const blogsWithUserIds = helper.initialBlogs.map(
      // eslint-disable-next-line comma-dangle
      (b) => ({ ...b, ...{ user: savedUser.body.id } })
    )
  ...
  describe('blogs recieve their id\'s and return', () => {
    ...
    test('all blogs have user assigned', async () => {
      const response = await api.get('/api/blogs')
      response.body.forEach((blog) => expect(blog.id).toBeDefined())
    })
  ...
  describe('adding a blog', () => {
    test('adding a blog without authorization fails with status 401', async () => {
      const newBlog = {
        title: 'Testing title',
        author: 'Testy Tester',
        url: 'https://test.test/',
    ...
    test('adding a blog correctly', async () => {
      const authenticationResponse = await api.post('/api/login').send({
        username: helper.initialUser.username,
        password: helper.initialUser.password,
      }).expect(200)

      const tokenBearer = 'bearer '.concat(authenticationResponse.body.token)
